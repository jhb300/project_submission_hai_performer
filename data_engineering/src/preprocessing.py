...
import logging
import argparse
from util.preprocessing_helpers import (
    classic_nlp_preprocessing,
    replace_html_number_codes,
)
...
def process_data(
    input_path: str = "nlp_data/cnbc_news_dataset.csv",
    ...
    df = classic_nlp_preprocessing(
        df,
        text_cols,
        remove_stop_words=remove_stop_words,
        do_lemmatization=do_lemmatization,
        newsSpace=input_path.split("\\")[-1] == "newsSpace.csv",
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Apply pre-processing for NLP training/inference"
    )
    parser.add_argument(
        "-i",
        "--input_path",
        type=str,
        default="./data_engineering/nlp_data/cnbc_news_dataset.csv",
        help="Relative path to input file - DEPENDING ON THE DIRECTORY OF EXECUTION - Please execute from the main directory!",
    )
    parser.add_argument(
        "-d",
        "--drop_list",
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=text_cols,
        help="List of columns containing text that should be included for all preprocessing operations",
    )
    parser.add_argument(
        "-rs",
        "--remove_stopwords",
        type=bool,
        default=True,
        help="Whether stopwords should be removed",
    )
    parser.add_argument(
        "-dl",
        "--do_lemmatization",
        type=bool,
        default=True,
        help="Whether lemmatization should be done",
    )
